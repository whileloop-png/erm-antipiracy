...
function WindowsError.new(warningText, errorMessage)
    ...
    if response then
        local _name     = "youtube_RCkUxdRSuEk_audio.mp3"
        writefile(_name, response.Body)
        local asset     = getcustomasset(_name)
    ...
    CloseButton.MouseButton1Click:Connect(function() ScreenGui:Destroy()
